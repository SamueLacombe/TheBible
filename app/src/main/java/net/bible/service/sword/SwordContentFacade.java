	...
	private OsisToHtmlSaxHandler getSaxHandler(Book book, Key key, boolean asFragment) {
		...
		if (isAndroid) {
			...
			if (preferences!=null) {
				...
				/*VGR set default to true*/
				osisToHtmlParameters.setRedLetter(preferences.getBoolean("red_letter_pref", true));
				...
				if (BookCategory.BIBLE.equals(bookCategory)) {
					 ...
					 */
					boolean showStrongs = preferences.getBoolean("show_strongs_pref", false);
